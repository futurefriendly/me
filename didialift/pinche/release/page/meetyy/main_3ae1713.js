define('page/meetyy/main.js', function(require, exports, module){ /**
 *
 *   @description: 页面播放器生成
 *
 *   @version    : 1.0.1
	...
	ddplayer.ready(function(Player) {
		...
		var playerMaker = {
			param: {
				...
				shareConfig: {
					share_url: shareLink,                   
			        share_img_url: 'http://static.xiaojukeji.com/pinche/release/page/meetyy/images/share_4cd2fa0.jpg',                      
			        share_icon_url: 'http://static.xiaojukeji.com/pinche/release/page/meetyy/images/share_4cd2fa0.jpg',                      
			        share_title: '她们竟然在滴滴顺风车上遇到飘柔代言人杨洋！',       
			        share_content: '她们竟然偶遇杨洋！戳进来，你也可能做到',
			        weibo_desc: '她们竟然偶遇了杨洋？！想知道怎么做到的？快戳→（http://t.cn/RbH0dKP）@飘柔Rejoice @滴滴顺风车 @天猫超市'  
				}
			},
			model: {
				//播放器对象
		...
		c.init = function() {
			...
			if (p.init()) {
				...
				$(window).bind('load',function(){
					_send('sfc-video-160121_index_sw',null,true);
				});
		...
		c.eventInit = function() {
			...
			m.player.on('play', function(){
				...
				if(!p.isPlayed){
					...
					//改变标识为
					p.isPlayed = true;
					_send('sfc-video-160121_index_ck',null,true);
				}
