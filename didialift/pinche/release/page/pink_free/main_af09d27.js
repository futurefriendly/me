define('page/pink_free/main.js', function(require, exports, module){ /**
 *
 *   @description: 页面播放器生成
 *
 *   @version    : 1.0.1
	...
	ddplayer.ready(function(Player) {
		...
		var playerMaker = {
			param: {
				...
				shareConfig: {
					share_url: shareLink,                   
			        share_img_url: 'http://static.xiaojukeji.com/pinche/release/page/pink_free/images/share_ed10ff9.jpg',                      
			        share_icon_url: 'http://static.xiaojukeji.com/pinche/release/page/pink_free/images/share_ed10ff9.jpg',                      
			        share_title: '粉色星期三，她们这样打动身边的TA！',       
			        share_content: '每个女生都有精彩的一面，看看她们靠什么打动身边的TA！',
			        weibo_desc: '粉色星期三，她们这样打动身边的TA！-滴滴 (来自@滴滴顺风车)'  
				}
			},
			model: {
				//播放器对象
		...
		c.init = function() {
			...
			if (p.init()) {
				...
				$(window).bind('load',function(){
					_send('sfc-video-160104_index_sw',null,true);
				});
		...
		c.eventInit = function() {
			...
			m.player.on('play', function(){
				...
				if(!p.isPlayed){
					...
					//改变标识为
					p.isPlayed = true;
					_send('sfc-video-160104_index_ck',null,true);
				}
